		...
		<div class="panel-body">
			...
			<div class="panel panel-default">
				<div class="panel-heading">
					<i class='fa fa-table'></i> Daftar Jawaban
				</div>
				<div class="panel-body no-padding table-responsive"  style="max-height: 400px;overflow: auto;">
					...
					<tbody>
						...
						<?php 
							...
							$checked=($d->is_correct)?'checked="checked"':'';		
						?>
						<tr>
							<td>
								#{{$i}} <input type="hidden" name="question_answer-id-[{{$d->id}}]" value="{{$d->id}}">
								<textarea id="{{$name}}" {{$required}} {{$readonly}} {{$disabled}} name="question_answer-answer-[{{$d->id}}]" class='wysiwygtext form-control' rows='5'>{{$d->answer}}</textarea>
							</td>
							<td>
								<input type='radio' {{$checked}} name="question_answer-is_correct-[{{$d->id}}]">
							</td>
						</tr>
						<?php $i++ ?>
						<?php endforeach;?>
